...
int main(int argc, char **argv)
{
    ...
    MPI_Bcast(&n_elements, 1, MPI_INT, 0, MPI_COMM_WORLD); // when sending single values we neeed to send the adress!!!!
    ...
    MPI_Bcast(buffer, n_elements, MPI_DOUBLE, 0, MPI_COMM_WORLD); // when sending list we don't need to send the adress!!!!
    ...
    std::cout << "Process with rank " << rank << " calculated the sum: " << sum << std::endl;
