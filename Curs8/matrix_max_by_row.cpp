...
int main(int argc, char **argv)
{
    ...
    int max_row[matrix_size];

    MPI_Gather(&max, 1, MPI_INT, max_row, 1, MPI_INT, 0, MPI_COMM_WORLD);

    if (rank == 0)
    {
        std::cout << "Process with rank: " << rank << " says: The max of the matrix is equal to " << std::endl;
